...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_admin(user_id):
        keyboard.insert(2, [InlineKeyboardButton("⚙️ لوحة التحكم 🛠️", callback_data="btn_admin")])
...
async def button_click_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    back_btn = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 العودة للقائمة", callback_data="btn_main_menu")]])

    if data_key == "btn_contestants":
        c_data = load_data().get("contestants", {})
        if not c_data:
            text = "⚠️ لا يوجد متسابقين حالياً."
        else:
            text = "📋 **قائمة المتسابقين:**\n\n"
        ...
        if sorted_c:
            leader = sorted_c[0][1]
            text += f"🏆 المتصدر: **{leader.get('name')}** بـ `{leader.get('votes')}` صوت"
        await query.edit_message_text(text, reply_markup=back_btn, parse_mode="Markdown")

    elif data_key == "btn_help":
        text = "ℹ️ **طريقة التصويت:**\n1. اشترك بالقناة أولاً.\n2. اضغط على زر التصويت أسفل منشور المتسابق بالقناة."
        await query.edit_message_text(text, reply_markup=back_btn, parse_mode="Markdown")
...
async def admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data_action == "admin_stats":
        ...
        data["contestants"] = {}
        save_data(data)
        await query.answer("🔄 تم تصفير كافة بيانات المسابقة!", show_alert=True)
        await admin_panel(update, context)
...
async def add_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    save_data(data)
    await update.message.reply_text(f"✅ تم إضافة `{new_admin_id}` كـ أدمن بنجاح!", parse_mode="Markdown")
...
async def process_vote(update: Update, context: ContextTypes.DEFAULT_TYPE, contestant_id: str):
    ...
    if not subscribed:
        ...
        msg = f"مرحباً بك {user.first_name}! 👋\n\nيجب الاشتراك بالقناة أولاً لفرز صوتك للمتسابق {contestant['name']}:\n👉 {CHANNEL_USERNAME}"
...
async def apply_vote(update: Update, context: ContextTypes.DEFAULT_TYPE, contestant_id: str, user, is_callback=False, skip_limits=False):
    ...
    if skip_limits:
        ...
        msg = f"⚡ [أدمن] تم احتساب صوتك للمتسابق {target_contestant['name']}!"
        if is_callback:
    ...
    msg = f"✅ تم احتساب التصويت بنجاح للمتسابق {target_contestant['name']}"
    if is_callback:
        await update.callback_query.edit_message_text(msg, parse_mode="Markdown")
    else:
        await update.message.reply_text(msg, parse_mode="Markdown")
...
async def view_voters(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("❌ أرسل الأمر هكذا: `/voters 1`", parse_mode="Markdown")
        return
    ...
    if not voters:
        ...
        for idx, v in enumerate(voters, 1):
            name = v.get("name", "مستخدم")
            usr = v.get("username", "بدون يوزر")
            formatted.append(f"• {idx}. {name} ({usr})")
        voters_str = "\n".join(formatted)
    
    c_name = contestant.get("name", "")
    c_votes = contestant.get("votes", 0)
    full_msg = f"📊 المصوتون لـ ({c_name}):\nالأصوات: {c_votes}\n\n{voters_str}"
...
async def set_votes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) < 2:
        await update.message.reply_text("❌ الاستخدام: `/setvotes 1 50`", parse_mode="Markdown")
        return
    ...
    await update.message.reply_text(f"✅ تم تعديل الأصوات إلى {new_votes}!")
...
async def main():
    ...
    # إعداد الأمر بسطر قصير آمن
    cmd_def = [BotCommand("start", "القائمة الرئيسية")]
    await application.bot.set_my_commands(cmd_def, scope=BotCommandScopeDefault())
